...
class LLMTrendDetectionLLMClient:
    ...
    def __init__(self, model: str = "gpt-5-mini", temperature: float = 0.0, api_key: Optional[str] = None, timeout: float = 180.0):
        self._model = model
...
class LLMTrendDetectionStrategy(Strategy):
    ...
    async def on_decision(self, ctx: Context, now: datetime) -> None:
        ...
        # Check if we have sufficient bars, retry with longer lookback if needed
        if not bars or len(bars) < min_bars_required:
            ctx.log(
                f"[LLM_Trend_Detection] Insufficient bars for {self._symbol}: {len(bars) if bars else 0} "
                f"(required: {min_bars_required}). Attempting to fetch more data..."
            )
            
            # Try fetching with a longer lookback period
            timeframe_upper = self._cfg.timeframe.upper()
            if timeframe_upper == "1D":
                # For daily, try up to 2 years of history
                extended_start = now - timedelta(days=730)
            elif timeframe_upper.endswith("H"):
                # For hourly, try up to 1 year of history
                extended_start = now - timedelta(days=365)
            else:
                # For other timeframes, try up to 180 days
                extended_start = now - timedelta(days=180)
            
            try:
                extended_bars = await self._data.get_bars(self._symbol, extended_start, now, self._cfg.timeframe)
                if extended_bars and len(extended_bars) >= min_bars_required:
                    bars = extended_bars
                    ctx.log(
                        f"[LLM_Trend_Detection] Successfully fetched {len(bars)} bars with extended lookback "
                        f"(required: {min_bars_required})"
                    )
                else:
                    # Still insufficient even with extended lookback
                    raise RuntimeError(
                        f"Insufficient bars for {self._symbol}: {len(extended_bars) if extended_bars else 0} "
                        f"(required: {min_bars_required}). "
                        ...
                        f"Tried fetching from {extended_start.date()} to {now.date()}."
                    )
            except Exception as e:
                # If fetching failed, raise an error
                raise RuntimeError(
                    f"Failed to fetch sufficient bars for {self._symbol}: {len(bars) if bars else 0} "
                    f"(required: {min_bars_required}). "
                    f"Indicator requirements: MA_short={self._cfg.ma_short}, "
                    f"MA_medium={self._cfg.ma_medium}, MA_long={self._cfg.ma_long}, "
                    f"RSI_length={self._cfg.rsi_length}, slope_window={self._cfg.slope_window}. "
                    f"Error during extended fetch: {e}"
                ) from e
