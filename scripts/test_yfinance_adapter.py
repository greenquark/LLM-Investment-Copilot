...
async def test_get_bars():
    ...
    print("=" * 60)
    print("⚠️  Note: Intraday data (1m, 5m, 15m, 30m) limited to last 30-60 days by yfinance")
    ...
    if _YFINANCE_DIRECT_AVAILABLE:
        ...
        try:
            ...
            if not test_df.empty:
                print(f"   ✅ Direct yfinance works! Got {len(test_df)} bars")
                print(f"      Date range: {test_df.index.min()} to {test_df.index.max()}")
                print(f"      Columns: {list(test_df.columns)}")
            else:
                print(f"   ⚠️  Direct yfinance returned empty DataFrame")
    ...
    try:
        ...
        if not _YFINANCE_DIRECT_AVAILABLE:
            print("❌ yfinance not available for direct testing")
    ...
    for timeframe in timeframes:
        ...
        try:
            ...
            if df is None or df.empty:
                print(f"   ⚠️  No bars returned (might be outside trading hours or invalid date range)")
                continue
            ...
            if bars:
                print(f"   ✅ Success! Retrieved {len(bars)} bars (RAW - no time normalization)")
...
async def test_get_bars_with_cache():
    ...
    print(f"\n📊 Testing with cache (timeframe: 1D)")
    print(f"   Symbol: {symbol}")
    print(f"   Date range: {start_date.date()} to {end_date.date()}")
    ...
    try:
        ...
        print(f"   ✅ Retrieved {len(bars1)} bars")
    ...
    try:
        ...
        if len(bars1) == len(bars2):
            print(f"   ✅ Cache verified: Same number of bars ({len(bars1)})")
        else:
            print(f"   ⚠️  Warning: Different number of bars (first: {len(bars1)}, second: {len(bars2)})")
...
async def test_get_option_chain():
    ...
    print("=" * 60)
    print("⚠️  NOTE: Option data is DAILY-LEVEL (snapshot, not minute-by-minute)")
    ...
    print(f"\n📊 Testing option chain (DAILY snapshot)")
    print(f"   Symbol: {symbol}")
    ...
    try:
        ...
        if contracts:
            print(f"   ✅ Success! Retrieved {len(contracts)} option contracts")
            ...
            print(f"\n   ⚠️  Data Type: DAILY snapshot (not minute-level)")
            print(f"   Data source: {adapter.last_data_source}")
        else:
            print(f"   ⚠️  No option contracts returned")
...
async def test_yfinance_direct_features():
    ...
    if not _YFINANCE_DIRECT_AVAILABLE:
        print("\n⚠️  yfinance not available for direct testing")
        print("   Install with: pip install yfinance")
        return
    ...
    try:
        ...
        if info:
            print(f"   ✅ Retrieved company info ({len(info)} fields)")
            ...
            print(f"   ⚠️  No company info returned")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        if financials is not None and not financials.empty:
            print(f"   ✅ Income Statement: {len(financials.columns)} periods, {len(financials)} line items")
        else:
            print(f"   ⚠️  Income Statement: No data")
        
        # Test balance sheet
        balance = await loop.run_in_executor(None, lambda: ticker.balance_sheet)
        if balance is not None and not balance.empty:
            print(f"   ✅ Balance Sheet: {len(balance.columns)} periods, {len(balance)} line items")
        else:
            print(f"   ⚠️  Balance Sheet: No data")
        
        # Test cash flow
        cashflow = await loop.run_in_executor(None, lambda: ticker.cashflow)
        if cashflow is not None and not cashflow.empty:
            print(f"   ✅ Cash Flow: {len(cashflow.columns)} periods, {len(cashflow)} line items")
        else:
            print(f"   ⚠️  Cash Flow: No data")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        if recommendations is not None and len(recommendations) > 0:
            print(f"   ✅ Recommendations: {len(recommendations)} analyst ratings")
            ...
            print(f"   ⚠️  Recommendations: No data")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        if holders is not None and len(holders) > 0:
            print(f"   ✅ Institutional Holders: {len(holders)} institutions")
            print(f"      Top holder: {holders.iloc[0].get('Holder', 'N/A')}")
        else:
            print(f"   ⚠️  Institutional Holders: No data")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        if income_stmt is not None and not income_stmt.empty:
            ...
            if 'Net Income' in income_stmt.index:
                ...
                print(f"   ✅ Earnings (from income_stmt): {len(net_income)} periods")
                if len(net_income) > 0:
                ...
                print(f"   ⚠️  Earnings: Net Income not found in income statement")
        else:
            print(f"   ⚠️  Earnings: No income statement data")
    except Exception as e:
        print(f"   ❌ Error: {e}")
...
async def test_timeframe_validation():
    ...
    for tf in invalid_timeframes:
        try:
            ...
            print(f"   {tf:6s} -> ✅ Correctly rejected: {str(e)[:50]}")
        except Exception as e:
            print(f"   {tf:6s} -> ⚠️  Unexpected error: {e}")
...
async def main():
    ...
    print("\n⚠️  IMPORTANT: Option data is DAILY-LEVEL only (snapshot, not minute-by-minute)")
    print("   For minute-level option data, use MarketData.app or other providers")
    ...
    await test_get_bars()
    await test_get_bars_with_cache()
    await test_get_option_chain()
    await test_yfinance_direct_features()
    
    print("\n" + "=" * 60)
    print("Test Suite Complete")
    print("=" * 60)
    print("\nSummary:")
    print("  ✅ Historical bars: Supports multiple timeframes (1m to 1mo)")
    print("  ✅ Option chains: DAILY-LEVEL only (current quotes for a date)")
    print("  ✅ Caching: Integrated with CachedDataEngine")
    print("  ✅ Direct yfinance: Company info, financials, dividends, etc. available")
    print("\n⚠️  Limitations:")
